...
export default function TaxNew() {
  ...
  const selectProperty = async (property: any) => {
    ...
    setFormData(prev => ({
      ...
      land_size: property.size?.toString() || '',
    ...
    try {
      const response = await taxService.getAssessments({
        search: property.id, // Backend should filter by property_id
        limit: 100,
      });
      
      const years = (response.data || [])
        .filter((a: any) => a.property_id === property.id)
        .map((a: any) => a.tax_year);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create assessment via backend
      const data = await taxService.createAssessment(payload);
