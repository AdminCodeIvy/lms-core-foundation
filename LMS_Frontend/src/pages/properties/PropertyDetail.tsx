...
export default function PropertyDetail() {
  ...
  const fetchPropertyDetail = async () => {
    try {
      ...
      setProperty(data);
      setBoundaries(data.property_boundaries);
      setPhotos(data.property_photos || []);
      setOwnership(data.property_ownership || []);
    } catch (error: any) {
  ...
  const fetchTaxAssessments = async () => {
    try {
      // Tax assessments will be included in property data or fetched separately
      // For now, skip or implement taxService if needed
      setTaxAssessments([]);
    } catch (error) {
