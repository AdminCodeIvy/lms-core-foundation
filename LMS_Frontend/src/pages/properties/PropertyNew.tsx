...
export default function PropertyNew() {
  ...
  const fetchLookups = async () => {
    try {
      ...
      // Format customer names - backend already provides formatted names
  ...
  const fetchSubDistricts = async (districtId: string) => {
    try {
      ...
      console.log('Sub-districts fetched:', { count: data?.length || 0, data });
    ...
    try {
      ...
      if (selectedImages.length > 0) {
        for (const image of selectedImages) {
          try {
            await propertyService.uploadPhoto(property.id, image);
          } catch (photoError) {
