  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      if (selectedType === 'CUSTOMER') {
        ...
        switch (selectedCustomerType) {
          case 'PERSON':
            ...
            sampleRow = {
              ...
              'Mothers Name *': 'Fatima Hassan',
              'Date of brith': '1990-05-15',
              'POB': 'Jigjiga',
              'Gender': 'MALE',
              'Nationality': 'Ethiopia',
              'Mobile Number 1': '+251912345678',
              'Email': 'ahmed.mohamed@example.com',
              'Type of ID': 'National ID Card',
        ...
        const instructions = [
          ['Person Customer Bulk Upload Instructions'],
          [''],
          ['REQUIRED FIELDS (marked with *):'],
          ['1. PROPERTY ID * - Unique property identifier (e.g., PID001)'],
          ['2. Full Name * - Complete name of the person'],
          ['3. Mothers Name * - Full name of mother'],
          [''],
          ['OPTIONAL FIELDS (no * required):'],
          ['- Date of brith: Date format YYYY-MM-DD (e.g., 1990-05-15)'],
          ['- POB: Place of birth'],
          ['- Gender: MALE or FEMALE (all caps)'],
          ['- Nationality: Country of nationality'],
          ['- Mobile Number 1: Format +XXX-XXX-XXX-XXX'],
          ...
          ['2. Only fields marked with * are required'],
          ['3. Optional fields can be left empty or contain "Not found"'],
          ['4. Maximum 1,000 rows per upload'],
          ['5. Do not modify column headers'],
          ['6. Save as .xlsx or .xls before uploading'],
          ['7. Customer ID will be auto-generated by the system'],
        ];
              ...
              if (selectedCustomerType === 'PERSON') {
                ...
                const getValue = (row: any, ...possibleKeys: string[]) => {
                  ...
                  for (const key of possibleKeys) {
                    if (row[key] !== undefined && row[key] !== null && row[key].toString().trim() !== '' && row[key].toString().trim() !== 'Not found' && row[key].toString().trim() !== 'Not Found') {
                      return row[key];
                    }
                  }
                  ...
                  for (const possibleKey of possibleKeys) {
                    for (const rowKey of rowKeys) {
                      if (rowKey.trim().toLowerCase() === possibleKey.toLowerCase()) {
                        if (row[rowKey] !== undefined && row[rowKey] !== null && row[rowKey].toString().trim() !== '' && row[rowKey].toString().trim() !== 'Not found' && row[rowKey].toString().trim() !== 'Not Found') {
                          return row[rowKey];
                        }
                      }
                    }
                ...
                // Validate only 3 required fields for PERSON (customer_id is auto-generated)
                if (!getValue(row, 'PROPERTY ID', 'Property ID', 'pr_id', 'PR-ID', 'pr-id', 'PR_ID')) {
                  messages.push('PROPERTY ID is required (column: PROPERTY ID, Property ID, or pr_id)');
                  status = 'error';
                }
                if (!getValue(row, 'Full Name', 'full_name', 'Full Name ', ' Full Name', 'full-name', 'Full_Name', 'fullname', 'name', 'Name')) {
                  messages.push('Full Name is required (column: Full Name, full_name, or Name)');
                  status = 'error';
                }
                if (!getValue(row, 'Mothers Name', 'mothers_name', 'mothers-name', 'Mothers_Name', 'Mother Name', 'mother_name')) {
                  messages.push('Mothers Name is required (column: Mothers Name, mothers_name, or Mother Name)');
                  status = 'error';
                }
                
                // Optional field validations - only validate format if provided
                const email = getValue(row, 'Email', 'email', 'E-mail', 'e_mail');
                if (email && email.toString().trim() && !email.toString().includes('@')) {
                  messages.push('Email format is invalid');
                  status = status === 'error' ? 'error' : 'warning';
                }
                
                const mobile1 = getValue(row, 'Mobile Number 1', 'mobile_number_1', 'mobile-number-1', 'Mobile_Number_1', 'phone1', 'Phone 1');
                if (mobile1 && mobile1.toString().trim() && !mobile1.toString().match(/^\+?[\d\s\-]{8,15}$/)) {
                  messages.push('Mobile Number 1 should be 8-15 digits (optional formatting allowed)');
                  status = status === 'error' ? 'error' : 'warning';
                }
                
                const mobile2 = getValue(row, 'Mobile Number 2', 'mobile_number_2', 'mobile-number-2', 'Mobile_Number_2', 'phone2', 'Phone 2');
                if (mobile2 && mobile2.toString().trim() && !mobile2.toString().match(/^\+?[\d\s\-]{8,15}$/)) {
                  messages.push('Mobile Number 2 should be 8-15 digits (optional formatting allowed)');
                  status = status === 'error' ? 'error' : 'warning';
                }
