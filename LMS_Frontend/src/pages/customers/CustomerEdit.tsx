...
const CustomerEdit = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {customer.customer_type === 'BUSINESS' && customer.business_data && <BusinessForm defaultValues={customer.business_data} onSubmit={handleSubmit} onCancel={() => navigate(`/customers/${id}`)} isSubmitting={isSubmitting} />}
      ...
      {customer.customer_type === 'MOSQUE_HOSPITAL' && customer.mosque_hospital_data && <MosqueHospitalForm defaultValues={customer.mosque_hospital_data} onSubmit={handleSubmit} onCancel={() => navigate(`/customers/${id}`)} isSubmitting={isSubmitting} />}
      {customer.customer_type === 'NON_PROFIT' && customer.non_profit_data && <NonProfitForm defaultValues={customer.non_profit_data} onSubmit={handleSubmit} onCancel={() => navigate(`/customers/${id}`)} isSubmitting={isSubmitting} />}
      ...
      {customer.customer_type === 'RENTAL' && customer.rental_data && <RentalForm defaultValues={{...customer.rental_data, date_of_birth: new Date(customer.rental_data.date_of_birth)}} onSubmit={handleSubmit} onCancel={() => navigate(`/customers/${id}`)} isSubmitting={isSubmitting} countries={countries} idTypes={ID_TYPES} />}
    </div>
  );
};
